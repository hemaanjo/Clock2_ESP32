...
void Grid_de_DE_clock2::setTime(int hour, int minute) {
  if(hour == -1 || minute == -1) {
	return;
  }
  ...
  for(int i = 0; i < 5; i++) {
	Led::ids[Led::getLedId(Grid_de_DE_clock2::time_it_is[i])].setRGB(Config::color_fg.r, Config::color_fg.g, Config::color_fg.b);
  }

  for(int m = 0; m < 12; m++) {
	if(Grid_de_DE_clock2::time_minutes[minute][m] >= 0) {
	  Led::ids[Led::getLedId(Grid_de_DE_clock2::time_minutes[minute][m])].setRGB(Config::color_fg.r, Config::color_fg.g, Config::color_fg.b);
	}
  }
  ...
  for(int h = 0; h < hourLimit; h++) {
	if(Grid_de_DE_clock2::time_hours[hour][h] >= 0) {
	  Led::ids[Led::getLedId(Grid_de_DE_clock2::time_hours[hour][h])].setRGB(Config::color_fg.r, Config::color_fg.g, Config::color_fg.b);
	}
  }
...
int Grid_de_DE_clock2::time_it_is[5] = {0, 1, 3, 4, 5}; // es ist

int Grid_de_DE_clock2::time_minutes[12][12] = {
...
int Grid_de_DE_clock2::time_hours[12][6] = {
